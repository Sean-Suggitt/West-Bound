...
func _ready() -> void:
	# Hide score sprites initially (score 0 = no display)
	p1_score_sprite.visible = false
	p2_score_sprite.visible = false
	
	# Update display with current scores
...
func _update_player_score_display(player: String, score: int) -> void:
	...
	if score == 0:
		...
		# Optional: Add a little animation when score increases
